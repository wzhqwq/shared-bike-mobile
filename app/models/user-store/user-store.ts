...
import { StatisticModel } from "../statistic/statistic"
...
export const UserStoreModel = types
  ...
  .actions((self) => ({
    ...
    async signUp(nickname: string, password: string) {
      const result: Response<string> = await self.environment.api.get('/auth/sign_up', { nickname, password })
      if (result.ok) {
        self.environment.updateJwt(result.data)
      }
      return result.ok
    },
    async signIn(nickname: string, password: string) {
      const result: Response<string> = await self.environment.api.get('/auth/sign_in', { nickname, password })
      if (result.ok) {
        self.environment.updateJwt(result.data)
      }
      return result.ok
    ...
    async registerAsCustomer() {
      const result: Response<string> = await self.environment.api.get('/auth/register_as_customer')
      if (result.ok) {
        self.environment.updateJwt(result.data)
      }
      return result.ok
    ...
    async checkRole() {
      const result: Response<string> = await self.environment.api.get('/auth/check_role')
      if (result.ok) {
        self.environment.updateJwt(result.data)
      }
      return result.ok
    ...
    async registerAs(type: number, name: string, phone: string) {
      const record = SignUpRequestModel.create({ type, name, phone })
      const result: Response<null> = await self.environment.api.get('/auth/request_to_be', record)
      if (result.ok) {
    ...
    async changeProfile(nickname: string, name?: string, phone?: string) {
      ...
      if (result.ok) {
        self.me.nickname = result.data.nickname
    ...
    async findBike(seriesNo: string) {
      ...
      if (result.ok) {
        self.bikeNow = result.data
      }
      return result.ok
    },
    async listMaintainersInSection(sectionId: number) {
      self.sectionIdNow = sectionId
      const result: Response<Maintainer[]> = await self.environment.api.post('/manager/section/maintainer/list', { section_id: sectionId })
      if (result.ok) {
    ...
    async getStatistics() {
      ...
      self.statistic = { ...bikeResult.data, ...billResult.data }
      return true
    },
  })) // eslint-disable-line @typescript-eslint/no-unused-vars
